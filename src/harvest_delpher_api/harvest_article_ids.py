...
def main():
    ...
    ARTICLE_ID_FOLDER.mkdir(exist_ok=True)
    ...
    for year in tqdm(range(start_year, end_year + 1)):
        for month in tqdm(range(start_month, end_month + 1)):
            output_path = ARTICLE_ID_FOLDER / f"article_meta_{year}_{month}.parquet"
            ...
            records_ls = []
            try:
                total_num_records = find_num_records(
            ...
            records_df = (
                pl.DataFrame(records_ls).select(list(columns_to_keep_and_rename.keys()))
            ).rename(columns_to_keep_and_rename)
            records_df = records_df.with_columns(
                pl.col("article_id")
                .str.replace(r"http://resolver.kb.nl/resolve\?urn=", "")
                .str.replace(r":ocr", ""),
