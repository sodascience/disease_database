...
#Define the regular expression for disease (cholera specifically) and location (Leiden)
disease_query = r"cholera"
location_query = r"le[iy]den"
...
# Querying only for cholera mentions in Leiden
...
# Plot for Leiden cholera mentions with key dates and smaller labels
plt = (
    p9.ggplot(
        qdf.with_columns(
...
p9.ggsave(plt, "img/leiden_cholera.png", width=8, height=5, dpi=300)
...
plt = (
    ...
    + p9.geom_line(colour="darkblue")
    + p9.scale_x_date(date_breaks="5 years", date_labels="%Y")
    + p9.coord_cartesian(ylim=(0, 0.1))  # Force the y-axis range from 0 to 0.1
    + p9.theme_linedraw()
    + p9.theme(legend_position="none")
    + p9.labs(
        title="Cholera and Crimean Fever Mentions in Amsterdam",
        y="Monthly normalized mentions",
        x=""
    )
)
...
# Save the plot as an image
p9.ggsave(plt, "img/amsterdam_cholera_krimkoorts.png", width=8, height=5, dpi=300)
